...
class WP_Job_Manager_Apply_With_Linkedin extends WPJM_Updater {
	...
	public function __construct() {
		// Define constants
		define( 'JOB_MANAGER_APPLY_WITH_LINKEDIN_VERSION', '2.1.3' );
		...
		if ( get_option( 'job_manager_allow_linkedin_applications_field' ) == '1' ) {
			add_filter( 'submit_job_form_fields', array( $this, 'allow_linkedin_field' ) );
			add_filter( 'job_manager_job_listing_data_fields', array( $this, 'allow_linkedin_field_admin' ) );
		}
	...
	public function frontend_scripts() {
		...
		wp_localize_script( 'wp-job-manager-apply-with-linkedin-js', 'apply_with_linkedin_params', array(
			'profile_public_i18n' => __( 'Sorry, your LinkedIn profile must be public in order to apply for this job listing.', 'wp-job-manager-apply-with-linkedin' )
		) );
	}
		...
		$settings['linkedin'] = array(
			...
			apply_filters(
				...
				array(
					...
					array(
						'name'		=> 'job_manager_allow_linkedin_applications_field',
						'std'		=> '',
						'label'		=> __( 'Allow LinkedIn Field', 'wp-job-manager-apply-with-linkedin' ),
						'cb_label'		=> __( 'Display Field', 'wp-job-manager-apply-with-linkedin' ),
						'desc'		=> __( 'Add a field to the job submission form, allowing employers to enable/disable LinkedIn applications', 'wp-job-manager-apply-with-linkedin' ),
						'type'		=> 'checkbox'
					),
				)
			)
	...
	/**
	 * Add allow LinkedIn field to the frontend job submission form
	 */
	public function allow_linkedin_field( $fields ) {
	  $fields['job']['allow_linkedin'] = array(
		'label'       => __( 'Allow LinkedIn Applications', 'wp-job-manager-apply-with-linkedin' ),
		'type'        => 'checkbox',
		'required'    => false,
		'placeholder' => '',
		'priority'    => 7,
		'description' => __( 'Do you want to allow employees to apply with their LinkedIn profile?', 'wp-job-manager-apply-with-linkedin' ),
	  );
	  return $fields;
	}

	/**
	 * Add allow LinkedIn field to the admin job listing meta
	 */
	public function allow_linkedin_field_admin( $fields ) {
	  $fields['_allow_linkedin'] = array(
		'label'       => __( 'Allow LinkedIn Applications', 'wp-job-manager-apply-with-linkedin' ),
		'type'        => 'checkbox',
		'required'    => false,
		'placeholder' => '',
	...
	public function allow_application( $job_id ) {
		if ( get_option( 'job_manager_allow_linkedin_applications_field' ) === '1' ) {
			return get_post_meta( $job_id, '_allow_linkedin', true );
		} else {
			return true;
		}
	}
	...
	public function apply_button( $apply ) {
		...
		// Only add the apply button if 'allow LinkedIn applications' was checked
